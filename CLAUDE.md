...
- `packages/extractor/` — CLI (`cli.ts`): repo discovery (scans for `.git` markers), TS/JS extraction via the TypeScript compiler API, viewer generation (embeds data + vendored d3 into the template). `github.ts` parses GitHub targets including the ref a `/tree/<ref>/` URL carries (pure, shared with `packages/site`) and `clone.ts` resolves a target to a local directory, shallow-cloning into the cache when it names a GitHub repo. One clone per (owner, repo, ref); the clone's basename is the repo name the map carries. `extract.ts` holds the tree-sitter WASM engine and `SITTER_LANGS`, a per-language config table covering the other 19 languages. `layout.ts` computes node positions at extract time.
