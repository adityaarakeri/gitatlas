...
- Releases are a pipeline instead of a manual `npm publish`: pushing a `v*` tag runs `.github/workflows/release.yml`, which checks the tag against `package.json` before building anything, runs the same gate as CI, packs the tarball, installs it globally, and exercises all six commands from a temp directory outside the checkout. The assertions are on artifacts, not exit codes, because a missing viewer template only warns: the generated HTML has to carry baked-in data and a vendored d3, `mcp` has to answer a real JSON-RPC `initialize` over stdio, and `site` has to answer `/healthz`, which is what proves the package-root walk survived the move into a global prefix. Publish is last and carries `--provenance`, passed on the command line rather than set in `publishConfig` so a manual publish from a laptop still works
- Homebrew: `packaging/homebrew/gitatlas.rb` installs the published npm tarball (`brew install adityaarakeri/gitatlas/gitatlas` once the tap is up), and the release workflow renders it into the tap with the new `url` and `sha256` after npm confirms the publish, hashing what the registry actually serves rather than the local pack output. `scripts/brew-formula.mjs` does the rendering and refuses to write unless both substitutions hit exactly once, since a new digest against an old url is a checksum failure at install time. The whole job is gated on `HOMEBREW_TAP_TOKEN` being set, so the tap stays optional and a release without it is npm-only
- Seven packaging tests (`packaging/test/`): the renderer rewrites exactly two lines and nothing else, refuses every malformed version and digest, and writes no file at all when a substitution does not hit once; the formula is checked against the Homebrew audit rules that cannot be run from Windows (desc length, no leading article, no repeated formula name), against the two facts the install depends on (`depends_on "node"` and `std_npm_args`, which is what keeps the `prepack` hook from firing on a tarball with no build script), and against `package.json` for license, homepage, and registry url drift. `packaging/` is now covered by the test runner, `tsconfig.json`, and eslint, and stays out of `dist/`
...
- `npm test` fixed on Node 22 again: the server import-side-effects test spawned its probe script through `--eval --input-type=module`, and named-export interop for a tsx-transpiled TS module is only reliably synthesized for an `import()` with a real caller module URL. Node 22 collapsed the module to just `default` under `--eval` (no caller URL) and the test read `createPlayground` as undefined; Node 24 carried the named export too, so the CI leg that would have caught this never ran red locally. The probe now runs from a real temporary file instead
